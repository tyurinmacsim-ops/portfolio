output "cluster_id" {
  ...
  value       = try(yandex_kubernetes_cluster.main.id, null)
}

output "cluster_name" {
  description = "Kubernetes cluster name."
  value       = try(yandex_kubernetes_cluster.main.name, null)
}
...
output "external_cluster_cmd" {
  ...
  EOF
  value       = var.public_access ? "yc managed-kubernetes cluster get-credentials --id ${yandex_kubernetes_cluster.main.id} --external" : null
}
...
output "internal_cluster_cmd" {
  ...
  EOF
  value       = var.public_access == false ? "yc managed-kubernetes cluster get-credentials --id ${yandex_kubernetes_cluster.main.id} --internal" : null
}
...
output "external_v4_address" {
  ...
  EOF
  value       = var.public_access == true ? yandex_kubernetes_cluster.main.master[0].external_v4_address : null
}
...
output "internal_v4_address" {
  description = <<EOF
    ...
    Note: Kubernetes internal cluster nodes are available from the virtual machines in the same VPC as cluster nodes.
  EOF
  value       = yandex_kubernetes_cluster.main.master[0].internal_v4_address
}
...
output "external_v4_endpoint" {
  ...
  EOF
  value       = var.public_access == true ? yandex_kubernetes_cluster.main.master[0].external_v4_endpoint : null
}
...
output "node_account_name" {
  ...
  EOF
  value       = local.use_existing_sa ? try("${var.node_account_name}-${var.cluster_name}", "") : try(yandex_iam_service_account.node_account[0].name, "")
}
...
output "node_account_id" {
  ...
  EOF
  value       = local.effective_node_sa_id
}
...
output "service_account_name" {
  ...
  EOF
  value       = local.use_existing_sa ? try("${var.service_account_name}-${var.cluster_name}", "") : try(yandex_iam_service_account.master[0].name, "")
}
...
output "service_account_id" {
  ...
  EOF
  value       = local.effective_master_sa_id
}
